...
export default function Dashboard() {
  ...
  const openSubject = (subjectName: string) => {
    ...
    const subjectKey = Object.keys(syllabusForGrade || {}).find((key) => cleanSubject.includes(key) || key.includes(cleanSubject));
                ...
                {[
                  ...
                  return (
                    <div key={label as string} className="bg-black/30 rounded-2xl p-4 border border-white/5">
                      ...
                      <p className="text-[10px] uppercase tracking-[0.2em] text-white/35 font-bold mt-1">{label as string}</p>
                    </div>
                  );
                })}
              </div>
              ...
              {recommendedCourses.map((course, index) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-[10px] uppercase tracking-[0.2em] font-black text-white/45">{course.level}</span>
                    <ChevronRight className="w-5 h-5 text-white/25 group-hover:text-primary group-hover:translate-x-1 transition-all" />
                  </div>
                  <h3 className="text-2xl font-black text-white mt-4">{course.title}</h3>
                  <p className="text-white/45 text-sm mt-2">{course.nextClass}</p>
                  ...
                  <p className="text-xs text-white/45 mt-2">{course.progress}% complete - {course.lessons} lessons</p>
                </motion.button>
              ))}
            </div>
          </section>
          ...
          <section className="lg:col-span-2 bg-white/5 border border-white/10 rounded-3xl p-6">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {liveClasses.map((item) => (
                <article key={item.id} className="bg-black/30 border border-white/10 rounded-3xl p-5">
                  <div className="flex items-center justify-between mb-5">
                    <span className={clsx('text-[10px] uppercase tracking-[0.2em] font-black px-3 py-1 rounded-full', item.status === 'Live' ? 'bg-red-500/15 text-red-300' : 'bg-primary/10 text-primary')}>
                      {item.status}
                    </span>
                    <PlayCircle className="w-6 h-6 text-white/45" />
                  </div>
                  <h3 className="font-black text-white text-lg leading-tight">{item.title}</h3>
                  <p className="text-white/45 text-sm mt-3">{item.mentor} - {item.time}</p>
                  <p className="text-white/35 text-xs mt-1">{item.duration} - {item.seats}</p>
                </article>
              ))}
            </div>
          </section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {interests.map((subject) => (
              <button key={subject} onClick={() => openSubject(subject)} className="bg-black/30 border border-white/10 rounded-3xl p-5 text-left hover:border-primary/35 transition-all group">
                ...
                <p className="text-white/45 text-sm mt-2">Chapters, AI lesson notes, and practice handoff.</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.94, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.94, y: 24 }} className="relative w-full max-w-3xl max-h-[86vh] overflow-hidden bg-black border border-white/10 rounded-3xl shadow-2xl flex flex-col">
              <div className="p-6 md:p-8 border-b border-white/10 bg-white/5 flex items-start justify-between gap-5">
                ...
                <button onClick={() => setSelectedSyllabus(null)} className="p-3 rounded-2xl bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-all">
                  <X className="w-6 h-6" />
                </button>
              </div>
